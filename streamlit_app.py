...
/* Eyebrow labels — condensed, spaced, uppercase. Used to name sections. */
.eyebrow {
  font-family: 'IBM Plex Mono', monospace;
  font-size: 0.68rem; font-weight: 600;
  letter-spacing: 0.14em; text-transform: uppercase;
  color: var(--slate); margin: 0 0 0.35rem 0;
}

/* Every number in this app is tabular mono. The numbers are the content. */
.stMetric [data-testid="stMetricValue"] {
  font-family: 'IBM Plex Mono', monospace !important;
  font-variant-numeric: tabular-nums;
  font-weight: 600 !important; font-size: 1.45rem !important;
  color: var(--ink) !important; letter-spacing: -0.01em;
}
[data-testid="stMetricLabel"] {
  font-family: 'IBM Plex Mono', monospace !important;
  font-size: 0.64rem !important; font-weight: 600 !important;
  letter-spacing: 0.11em !important; text-transform: uppercase;
...
.card {
  ...
  border-radius: 2px; padding: 1rem 1.15rem; margin-bottom: 0.75rem;
}
.card--wind { border-left: 3px solid var(--wind); }
.card--route { border-left: 3px solid var(--route); }
...
.rowline {
  ...
  padding: 0.34rem 0; border-bottom: 1px solid var(--rule);
  font-size: 0.86rem;
}
.rowline:last-child { border-bottom: none; }
.rowline .k { color: var(--slate); }
.rowline .v {
  font-family: 'IBM Plex Mono', monospace;
...
.pill {
  ...
  color: var(--slate); margin-right: 0.3rem;
}
.pill--on { background: var(--route); color: #fff; border-color: var(--route); }
.pill--warn { background: var(--wind); color: #fff; border-color: var(--wind); }

.stTabs [data-baseweb="tab-list"] { gap: 0.15rem; border-bottom: 1px solid var(--rule); }
...
with st.sidebar:
    ...
    st.divider()
    eyebrow("Filter routes")
    max_total = st.slider("Max total ride (miles)", 10, 110, 110, step=5)
...
hcol = st.columns([3, 2])
with hcol[0]:
    st.markdown("<h1 style='margin-bottom:0.1rem'>NYC Ride Finder</h1>", unsafe_allow_html=True)
    st.markdown(
        f"<p class='eyebrow'>Starting from {SP.label} · {len(VISIBLE)} of {len(ROUTES)} routes</p>",
        unsafe_allow_html=True,
    )
with hcol[1]:
    if NOW is not None:
        _now_rows = rows([
            ("Now", "{:.0f}°F, feels {:.0f}°".format(NOW.temp, NOW.feels)),
            ("Wind", "{:.0f} mph from {}, gust {:.0f}".format(
                NOW.wind, compass_point(NOW.wind_dir), NOW.gust)),
            ("Riding conditions", "{} · {:.0f}/100".format(
                score_label(NOW.score), NOW.score)),
        ])
        st.markdown(
            "<div class='card' style='padding:0.6rem 0.9rem'>" + _now_rows + "</div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            "<div class='card card--wind'><span class='eyebrow'>Forecast unavailable</span>"
            "<div style='font-size:0.85rem;color:#5A6570'>Weather could not be reached. "
            "Everything else still works.</div></div>", unsafe_allow_html=True,
        )
...
TAB_TODAY, TAB_ROUTE, TAB_WX, TAB_FUEL = st.tabs(
    ["Today", "Route detail", "7-day forecast", "Zones & fueling"]
)
...
with TAB_TODAY:
    if NOW is None:
        ...
        with left:
            ...
            if head > 4:
                st.markdown(
                    f"<div class='card card--wind'><span class='eyebrow'>Read</span>"
                    f"<div style='font-size:0.88rem'>You ride out into "
                    f"<b>{head:.0f} mph of headwind</b> and get pushed home. That is the right "
                    f"way round — the hard part happens on fresh legs.</div></div>",
                    unsafe_allow_html=True)
            elif head < -4:
                st.markdown(
                    f"<div class='card card--wind'><span class='eyebrow'>Read</span>"
                    f"<div style='font-size:0.88rem'>Tailwind out, <b>{abs(head):.0f} mph "
                    f"headwind on the way home</b>, on tired legs. Consider riding this one "
                    f"in reverse, or budget extra time and food for the return.</div></div>",
                    unsafe_allow_html=True)
            else:
                st.markdown(
                    f"<div class='card card--route'><span class='eyebrow'>Read</span>"
                    f"<div style='font-size:0.88rem'>Mostly crosswind at "
                    f"<b>{cross:.0f} mph</b>. Little help either direction, but watch for gusts "
                    f"on exposed bridges and open water.</div></div>",
                    unsafe_allow_html=True)

        with right:
            eyebrow("Best riding windows, next 7 days")
            ...
            for _, w in best.iterrows():
                pct = int(w["score"])
                bar_c = "#0E7C86" if pct >= 70 else "#FF5A1F" if pct < 45 else "#5A6570"
                st.markdown(
                    f"<div class='card' style='padding:0.55rem 0.85rem;margin-bottom:0.45rem'>"
                    f"<div style='display:flex;justify-content:space-between;align-items:baseline'>"
                    f"<span class='datum' style='font-size:0.95rem'>"
                    f"{w['time'].strftime('%a %d %b · %-I%p')}</span>"
                    f"<span class='datum' style='color:{bar_c}'>{score_label(w['score'])}</span></div>"
                    f"<div style='height:4px;background:#EDEFEA;margin:0.4rem 0'>"
                    f"<div style='height:4px;width:{pct}%;background:{bar_c}'></div></div>"
                    f"<div style='font-size:0.78rem;color:#5A6570;font-family:IBM Plex Mono,monospace'>"
                    f"{w['temp']:.0f}°F · {w['wind']:.0f} mph {compass_point(w['wind_dir'])} · "
                    f"gust {w['gust']:.0f} · {w['precip_prob']:.0f}% rain</div></div>",
                    unsafe_allow_html=True,
                )
            st.caption(
    ...
    st.divider()
    eyebrow("Routes ranked for today")
    if NOW is not None:
        tbl = []
        for r in VISIBLE:
            ...
            tbl.append({
                ...
                "Hours": round(r.total_from(SP) / avg_mph, 1),
                "Wind on route": ("head" if h > 2 else "tail" if h < -2 else "cross"),
                "Shelter": r.exposure.split("—")[0].strip(),
...
with TAB_ROUTE:
    ...
    if r.ferry_needed:
        pills.append("<span class='pill'>ferry</span>")
    pills.append(f"<span class='pill'>{r.difficulty}</span>")
    pills.append(f"<span class='pill'>open road {6 - r.interruption_score}/5</span>")
    st.markdown("".join(pills), unsafe_allow_html=True)

    m = st.columns(5)
    m[0].metric("Total", f"{total:.0f} mi")
    m[1].metric("Approach", f"{approach:.1f} mi")
    m[2].metric("Elevation", f"{r.elevation_ft:,} ft")
    m[3].metric("Moving time", f"{int(dur)}h {int(round((dur % 1) * 60)):02d}m")
    m[4].metric("Heading out", f"{heading:.0f}° {compass_point(heading)}")
    ...
    with mc[1]:
        ...
        if NOW is not None:
            ...
            st.markdown(
                "<div class='card' style='padding:0.55rem 0.8rem'>" + rows([
    ...
    with a:
        ...
        st.markdown(f"<div class='card card--route'>{r.approach.get(start_key, '')}</div>",
                    unsafe_allow_html=True)
    ...
    with b:
        eyebrow("The numbers")
        st.markdown("<div class='card'>" + rows(r.stats) + "</div>", unsafe_allow_html=True)

        eyebrow("Conditions")
        st.markdown("<div class='card'>" + rows([
            ("Surface", r.surface),
    ...
    for i, s in enumerate(r.stops):
        with scols[i % 2]:
            st.markdown(
                f"<div class='card' style='padding:0.6rem 0.85rem'>"
                f"<span class='datum' style='font-size:0.9rem'>{s.name}</span>"
                f"<div style='font-size:0.8rem;color:#5A6570;margin-top:0.2rem'>{s.what}</div>"
...
with TAB_WX:
    if not WX["ok"] or HOURLY is None:
        ...
        eyebrow(f"Seven days at {SP.label}")
        ...
        for col, (_, d) in zip(cols, agg.iterrows()):
            sc = d["day_score"]
            c = "#0E7C86" if sc >= 70 else "#FF5A1F" if sc < 45 else "#5A6570"
            with col:
                st.markdown(
                    f"<div class='card' style='padding:0.6rem 0.5rem;text-align:center'>"
                    f"<div class='eyebrow' style='margin-bottom:0.3rem'>"
                    f"{pd.Timestamp(d['date']).strftime('%a')}</div>"
                    f"<div class='big' style='font-size:1.35rem;color:{c}'>{sc:.0f}</div>"
                    f"<div style='font-size:0.68rem;color:#5A6570;font-family:IBM Plex Mono,monospace;"
                    f"margin-top:0.3rem'>{d['high']:.0f}/{d['low']:.0f}°F<br/>"
                    f"{d['wind']:.0f} mph<br/>gust {d['gust']:.0f}<br/>{d['rain']:.0f}% rain</div>"
                    f"</div>", unsafe_allow_html=True)
...
with TAB_FUEL:
    ...
    st.markdown(
        f"<div class='card card--wind'><span class='eyebrow'>Why the number on your watch "
        f"may not mean what a coach means</span>"
    ...
    with fc[1]:
        st.metric("Estimated time", f"{int(fdur)}h {int(round((fdur % 1) * 60)):02d}m")
    ...
    e = st.columns(4)
    e[0].metric("Target HR", f"{fm['hr']} bpm")
    e[1].metric("Energy cost", f"{fm['kcal_hr']:.0f} kcal/h")
    e[2].metric("Ride total", f"{fm['kcal_total']:.0f} kcal")
    e[3].metric("Fuel mix", f"{fm['carb_frac'] * 100:.0f}% carbs")
    ...
    g = st.columns(4)
    g[0].metric("Carbs burned", f"{fm['ox']:.0f} g/h")
    g[1].metric("Carbs to eat", f"{fm['intake']:.0f} g/h")
    g[2].metric("Total on bike", f"{fm['total_intake']:.0f} g")
    g[3].metric("Fluid", f"{fm['fluid_total']:.1f} L")
